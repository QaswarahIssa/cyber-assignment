...
# التنسيق الشامل لتطبيق كافة التعديلات المطلوبة
st.markdown("""
    <style>
    /* 1. اتجاه الصفحة العام ليكون RTL للغة العربية */
    .stApp { 
    ...
    /* 2. جعل جميع النصوص والأسئلة وعناوين الإدخال باللون الأبيض الواضح جداً */
    html, body, [class*="css"], div, p, span, label, .stWidgetLabel, .stRadio label, .stSelectbox label, .stNumberInput label, .stTextInput label {
    ...
    /* 4. إرجاع عرض الأكواد البرمجية (st.code) من اليسار لليمين LTR لتصبح مقروءة */
    ...
    /* 5. إزالة المستطيلات الفارغة واستبدالها بخط فاصل أبيض */
    hr {
    ...
    /* 6. إصلاح القوائم المنسدلة وخانات الإدخال لتظهر بنص أبيض بخلفية داكنة */
